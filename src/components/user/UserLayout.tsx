...
const UserLayout = ({ children, title }: UserLayoutProps) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      {/* Header */}
      <header className="bg-white shadow-[0px_8px_32px_0px_rgba(31,38,135,0.15)] border-b fixed w-full z-10">
        <div className="max-w-[1280px] mx-auto px-4 h-16 flex items-center justify-between">
          {/* Mobile menu button */}
          <button 
            onClick={() => setSidebarOpen(!sidebarOpen)}
            className="lg:hidden text-gray-500 hover:text-[#9b87f5]"
          >
          ...
          <div className="flex-1 flex items-center justify-center lg:justify-start">
            <Link to="/dashboard" className="font-bold text-xl text-[#9b87f5]">
          ...
          <div className="flex items-center space-x-4">
            <Link to="/" className="mr-2 text-gray-600 hover:text-[#9b87f5]">
              <Home className="h-5 w-5" />
            </Link>
            
            <button className="text-gray-600 hover:text-[#9b87f5] relative">
              <Bell className="h-5 w-5" />
              <span className="absolute -top-1 -right-1 bg-[#7E69AB] text-white text-xs rounded-full h-4 w-4 flex items-center justify-center">2</span>
            </button>
      ...
      <div className="flex flex-1 pt-16 max-w-[1280px] mx-auto w-full">
        ...
        <aside 
          ...
          } lg:translate-x-0 fixed lg:relative lg:block z-30 w-64 h-[calc(100vh-4rem)] bg-white border-r shadow-[0px_8px_32px_0px_rgba(31,38,135,0.15)] transition-transform duration-300 ease-in-out`}
        >
          <nav className="p-4 space-y-1 overflow-y-auto h-full">
            ...
            <div className="pt-8 mt-4 border-t">
              <div className="flex flex-col items-center text-center p-4 bg-[#9b87f5]/5 rounded-lg">
                <Heart className="h-8 w-8 text-[#9b87f5] mb-2" />
                <h3 className="font-medium">Need Help?</h3>
                <p className="text-sm text-gray-600 mt-1">Contact our support team for assistance</p>
                <Button size="sm" className="mt-3 w-full bg-[#9b87f5] hover:bg-[#7E69AB] text-white" asChild>
                  <Link to="/contact">
